...
const FormUser = ({ createNewUser, updateInfo, updateUserById, setUpdateInfo, closeForm, setCloseForm }) => {
  ...
  return (
    <div onClick={handleCloseForm} className={`formUser-wrapper ${closeForm && 'close-form'}`}>
      <form onClick={e => e.stopPropagation()} className='formUser' onSubmit={handleSubmit(submit)}>
        <h2 className='formUser__title'>{updateInfo ? 'Update User' : 'New Users'}</h2>
        ...
        <div className='formUser__group'>
          <label className='formUser__label' htmlFor="image_url">Update Url Image</label>
          <input className='formUser__input' {...register("image_url")} type="text" id="image_url" />
        </div>
